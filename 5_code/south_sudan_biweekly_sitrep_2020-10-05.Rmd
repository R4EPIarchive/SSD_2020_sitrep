---
title: "Internal MSF OCA South Sudan Mission Epi Biweekly SitRep"
output:
  html_document:
    df_print: paged
  word_document:
...
reporting_week_last_year <- yearweek("2019 W39")
...
## unity and mundri
unity_mundri_nutrition_raw <- rio::import(current_nutrition_unity_mundri,
                                          which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Leer 
leer_covid_raw <- rio::import(current_covid_leer,
                              which = "linelist")
...
## Mundri AWD
### 2019
mundri_awd_2019_opd_raw <- rio::import(raw_awd_mundri_2019,
                              which = "opd") %>%
  ## make all variable names lower case
  janitor::clean_names()


mundri_awd_2019_cbmc_raw <- rio::import(raw_awd_mundri_2019,
                              which = "cbmc") %>%
  ## make all variable names lower case
  janitor::clean_names()
...
## 2020
mundri_awd_opd_raw <- rio::import(current_raw_awd_mundri, 
                              which = "opd") %>% 
  ## make all variable names lower case
  janitor::clean_names()


mundri_awd_cbmc_raw <- rio::import(current_raw_awd_mundri, 
                              which = "cbmc") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Mundri malaria
### 2019
mundri_mal_2019_opd_raw <- rio::import(raw_malaria_mundri_2019,
                              which = "opd") %>%
  ## make all variable names lower case
  janitor::clean_names()


mundri_mal_2019_cbmc_raw <- rio::import(raw_malaria_mundri_2019,
                              which = "cbmc") %>%
  ## make all variable names lower case
  janitor::clean_names()
...
## 2020
mundri_mal_opd_raw <- rio::import(current_raw_malaria_mundri, 
                              which = "opd") %>% 
  ## make all variable names lower case
  janitor::clean_names()


mundri_mal_cbmc_raw <- rio::import(current_raw_malaria_mundri, 
                              which = "cbmc") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
bentiu_awd_mal_raw <- bentiu_awd_mal_raw %>% 
  mutate(site ="bentiu") %>% 
  mutate(iccm_acute_watery_diarrhoea_ages_5y = 0,
         iccm_acute_watery_diarrhoea_ages_5y_2 = 0)
...
ben_lan_awd_mal <- bind_rows(bentiu_awd_mal_raw, lankien_awd_mal_raw) %>% 
  ...
  select(year_week = periodname,
         ...
         iccm_acute_watery_diarrhoea_ages_5y,
         iccm_acute_watery_diarrhoea_ages_5y_2,
         site) %>% 
  ## create yearweek variable 
  mutate(epiweek = yearweek(year_week)) %>% 
  rowwise() %>% 
  ## add up total awd and total malaria cases
  mutate(all_awd = sum(awd_under5, awd_over5,
                       iccm_acute_watery_diarrhoea_ages_5y, 
                       iccm_acute_watery_diarrhoea_ages_5y_2, 
                       na.rm = TRUE),
         iccm_awd = sum(iccm_acute_watery_diarrhoea_ages_5y,
                        iccm_acute_watery_diarrhoea_ages_5y_2, 
                        na.rm = TRUE),
         all_awd_under5 = sum(awd_under5, iccm_acute_watery_diarrhoea_ages_5y, 
                              na.rm = TRUE),
         all_awd_over5 = sum(awd_over5, iccm_acute_watery_diarrhoea_ages_5y_2,
                             na.rm = TRUE),
         all_malaria = sum(malaria_under5, malaria_over5, 
                           iccm_rdt_positive_falciparum_ages_5y,
                           iccm_rdt_positive_falciparum_ages_5y_2, na.rm = TRUE),
         all_malaria_under5 = sum(malaria_under5, 
                                  iccm_rdt_positive_falciparum_ages_5y, na.rm = TRUE),
         all_malaria_over5 = sum(malaria_over5,
                                 iccm_rdt_positive_falciparum_ages_5y_2, na.rm = TRUE),
         opd_malaria = sum(malaria_under5, malaria_over5, na.rm = TRUE),
         iccm_malaria = sum(iccm_rdt_positive_falciparum_ages_5y, 
                            iccm_rdt_positive_falciparum_ages_5y_2, na.rm = TRUE)) %>% 
...
ben_lan_awd_wide <- ben_lan_awd_mal %>% 
  ...
  select(epiweek, site, all_awd_under5, all_awd_over5) %>% 
  pivot_longer(cols = c("all_awd_under5", "all_awd_over5"),
               names_to = "awd_age",
               values_to = "count")
...
ben_lan_awd_19_20_ma <- ben_lan_awd_mal %>% 
  ...
  mutate(ma_awd_4wk = round(slide_dbl(all_awd, ~mean(.x, na.rm = TRUE), 
                                      .before = 3 ), digits = 0))
...
unity_awd_19_20_wide <- unity_awd_19_20_cl %>% 
  ## rename awd columns to match bentiu/lankien dataset
  rename(all_awd_under5 = awd_under5,
         all_awd_over5 = awd_over5) %>% 
  ## select the variables to keep
  select(epiweek, site, all_awd_under5, all_awd_over5) %>% 
  pivot_longer(cols = c("all_awd_under5", "all_awd_over5"),
               names_to = "awd_age",
...
## Mundri 2019
## OPD
mundri_awd_2019_opd_cl <- mundri_awd_2019_opd_raw  %>%
  mutate(site = "mundri") %>%
  ## create a year variable
  mutate(year = 2019) %>%
  ## create an epiweek variable as aweek object
...
# ## cbmc
mundri_awd_2019_cbmc_cl <- mundri_awd_2019_cbmc_raw %>%
  mutate(site = "mundri") %>%
  ## create a year variable
  mutate(year = 2019) %>%
  ## create an epiweek variable as aweek object
  ...
  ## select variables of interest and relabel
  select(epiweek,
         cbmc_awd_under5 = under5,
         cbmc_awd_over5 = over5
         )



### join the 2019 OPD and CBMC datsets
 mundri_awd_2019_opd_cbmc_cl <- mundri_awd_2019_opd_cl %>%
   left_join(mundri_awd_2019_cbmc_cl, by = "epiweek") %>%
   ## add total value of awd cases by age group
   mutate(all_awd_under5 = under5 + cbmc_awd_under5,
          all_awd_over5 = over5 + cbmc_awd_over5) %>%
   ## add total value of awd cases
  mutate(all_awd = all_awd_under5 + all_awd_over5) %>%
  ## select variables of interest
  select(epiweek, site, all_awd_under5, all_awd_over5, all_awd)


## Mundri 2020
## OPD
mundri_awd_opd_cl <- mundri_awd_opd_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2020) %>% 
  ## create an epiweek variable as aweek object
...
## cbmc
mundri_awd_cbmc_cl <- mundri_awd_cbmc_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2020) %>% 
  ## create an epiweek variable as aweek object
...
### join the 2020 OPD and CBMC datsets
mundri_awd_opd_cbmc_cl <- mundri_awd_opd_cl %>% 
  left_join(mundri_awd_cbmc_cl, by = "epiweek") %>% 
  ## add total value of awd cases by age group
  mutate(all_awd_under5 = under5 + cbmc_awd_under5,
         all_awd_over5 = over5 + cbmc_awd_over5) %>% 
  ## add total value of awd cases
...
## Join 2019 and 2020 datasets
 mundri_awd_opd_cbmc_19_20_cl <- bind_rows(mundri_awd_2019_opd_cbmc_cl,
                                           mundri_awd_opd_cbmc_cl)
 


## Make wide dataset for malaria
mundri_awd_opd_cbmc_19_20_cl_wide <-  mundri_awd_opd_cbmc_19_20_cl %>%
  ## select the variables to keep
  select(epiweek, site, all_awd_under5, all_awd_over5) %>%
  pivot_longer(cols = c("all_awd_under5", "all_awd_over5"),
               names_to = "awd_age",
...
## create a dataset for the 4 week moving \naverage
mundri_awd_opd_cbmc_19_20_ma <- mundri_awd_opd_cbmc_19_20_cl %>%
  ##select variables of interest
  select(epiweek, site, all_awd) %>%
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek, key = site) %>%
...
<!-- ### Combine all wide datasets for AWD and the 4 week moving average -->

```{r combine_all_awd_wide_data}
## combine the various wide case datasets
ssudan_awd_wide <- bind_rows(mundri_awd_opd_cbmc_19_20_cl_wide,
                             unity_awd_19_20_wide, 
                             ben_lan_awd_wide )

## combine the project specific 4 week moving \naverage datasets
ssudan_awd_4ma <- bind_rows(mundri_awd_opd_cbmc_19_20_ma, unity_awd_19_20_ma,
...
<!-- - make the 4 week moving average -->
```{r mal_ben_lan}
## Make wide dataset for malaria
ben_lan_mal_wide <- ben_lan_awd_mal %>% 
  ## select the variables to keep
  select(epiweek, site, all_malaria_under5, all_malaria_over5) %>% 
  pivot_longer(cols = c("all_malaria_under5", "all_malaria_over5"),
...
ben_lan_mal_ma <- ben_lan_awd_mal %>%
  ...
  group_by(site) %>%
  ## create a moving \naverage variable (deals with missings)
  mutate(ma_malaria_4wk = round(slide_dbl(all_malaria, ~mean(.x, na.rm = TRUE), 
                                          .before = 3 ), digits = 0))
...
## select variables of interest and relabel
  select(epiweek, 
         all_malaria_under5 = under5,
         all_malaria_over5 = over5,
         site,
         year)
...
unity_malaria_19_20_cl <- bind_rows(unity_malaria_2019_cl, unity_malaria_cl) %>% 
  ## add total value of awd cases
  mutate(all_malaria = all_malaria_under5 + all_malaria_over5)
...
## OPD data
## Add a site variable to each dataset
mundri_malaria_2019_opd_cl <- mundri_mal_2019_opd_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2019) %>% 
  ## create an epiweek variable as aweek object
...
## CBMC data
## Add a site variable to each dataset
mundri_malaria_2019_cbmc_cl <- mundri_mal_2019_cbmc_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2019) %>% 
  ## create an epiweek variable as aweek object
...
### join the 2019 OPD and CBMC datsets
 mundri_malaria_2019_opd_cbmc_cl <- mundri_malaria_2019_opd_cl %>%
   left_join(mundri_malaria_2019_cbmc_cl, by = "epiweek") %>%
   ## add total value of awd cases by age group
   mutate(all_malaria_under5 = under5 + cbmc_awd_under5,
          all_malaria_over5 = over5 + cbmc_awd_over5) %>%
   ## add total value of awd cases
  mutate(all_malaria = all_malaria_under5 + all_malaria_over5) %>%
  ## select variables of interest
  select(epiweek, site, all_malaria_under5, all_malaria_over5, all_malaria)
...
## OPD
## Add a site variable to each dataset
mundri_malaria_opd_cl <- mundri_mal_opd_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2020) %>% 
  ## create an epiweek variable as aweek object
...
## CBMC data
## Add a site variable to each dataset
mundri_malaria_cbmc_cl <- mundri_mal_cbmc_raw %>%
  mutate(site = "mundri") %>% 
  ## create a year variable
  mutate(year = 2020) %>% 
  ## create an epiweek variable as aweek object
...
### join the 2020 OPD and CBMC datsets
 mundri_malaria_2020_opd_cbmc_cl <- mundri_malaria_opd_cl %>%
   left_join(mundri_malaria_cbmc_cl, by = "epiweek") %>%
   ## add total value of awd cases by age group
   mutate(all_malaria_under5 = under5 + cbmc_awd_under5,
          all_malaria_over5 = over5 + cbmc_awd_over5) %>%
   ## add total value of awd cases
...
## Join 2019 and 2020 datasets
 mundri_awd_opd_cbmc_19_20_cl <- bind_rows(mundri_malaria_2019_opd_cbmc_cl,
                                           mundri_malaria_2020_opd_cbmc_cl)



## Make wide dataset for malaria
mundri_malaria_opd_cbmc_19_20_cl_wide <-  mundri_awd_opd_cbmc_19_20_cl  %>% 
  ## select the variables to keep
  select(epiweek, site, all_malaria_under5, all_malaria_over5) %>% 
  pivot_longer(cols = c("all_malaria_under5", "all_malaria_over5"),
               names_to = "malaria_age",
...
## Create dataset with moving \naverages
mundri_malaria_opd_cbmc_19_20_ma <- mundri_awd_opd_cbmc_19_20_cl %>% 
  ## add 4 moving \naverages for malaria and malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek, key = site) %>% 
  group_by(site) %>% 
...
<!-- ### Combine all wide datasets for malaria and the 4 week moving average -->

```{r combine_all_malaria_wide_data}
## combine the various wide case datasets
ssudan_malaria_wide <- bind_rows(mundri_malaria_opd_cbmc_19_20_cl_wide,
                             unity_malaria_19_20_wide, 
                             ben_lan_mal_wide )

## combine the project specific 4 week moving \naverage datasets
ssudan_malaria_4ma <- bind_rows(mundri_malaria_opd_cbmc_19_20_ma, unity_malaria_19_20_ma,
                            ben_lan_mal_ma)
...
combined_resp_failure <- bind_rows(bentiu_resp_mort_cl, lankien_resp_mort_cl) %>% 
  group_by(site) %>% 
  ## add 4 week moving \naverage
  mutate(ma_resp_mort_4week = round(slide_dbl(resp_failure, ~mean(.x, na.rm = TRUE), 
                                              .before = 3 ), digits = 0))
...
ben_lan_itfc_cl <- bind_rows(bentiu_itfc_cl,lankien_itfc_cl) %>% 
  ...
  filter(epiweek <= reporting_week) %>% 
  mutate(itfc_total_admissions = if_else(is.na(itfc_total_admissions), 0, itfc_total_admissions))
...
ben_lan_itfc_ma <- ben_lan_itfc_cl %>% 
  ...
  group_by(site) %>% 
  ## create a moving \naverage variable (deals with missings)
  mutate(ma_itfc_4wk = round(slide_dbl(itfc_total_admissions,
                                 ~mean(.x, na.rm = TRUE), .before = 3 ), digits = 0))
...
<!-- ## COVID data -->
<!-- - add bentiu, lankien, leer site variables -->


```{r covid_data_combining}

## add a bentiu, lankien and leer site variable to each linelist
bentiu_covid_raw <- bentiu_covid_raw %>% 
  mutate(site = "bentiu")

## Add lankien site variable
lankien_covid_raw <- lankien_covid_raw %>% 
  mutate(site = "lankien") %>% 
  ## remove the duplicate entry
  filter(MSF_N_Patient != "CD61")

## Add Leer Site variable
leer_covid_raw <- leer_covid_raw %>% 
  mutate(site = "leer") %>% 
  ## temporarily make date of consultation the reporting date
  mutate(report_date = MSF_date_consultation)

## combine bentiu, lankien and leer
covid_data_raw <- bind_rows(bentiu_covid_raw, lankien_covid_raw, leer_covid_raw) %>% 
  ##make all variable names lower case
  janitor::clean_names()
```
...
<!-- - create an epiweek variable -->
<!-- - convert ageonset to a number -->
<!-- - create age_years and age group variables -->


```{r covid_cleaning_data}
...
bentiu_last_current <- bentiu_last_current %>% 
  ...
  mutate(four_week_roll = round(slide_dbl(total_deaths, ~mean(.x, na.rm = TRUE), 
                                          .before = 3 ), digits = 0))
...
ajs_poc_cl_grp <- ajs_poc_cl %>% 
  ...
  mutate(four_week_roll = round(slide_dbl(n, ~mean(.x, na.rm = TRUE), 
                                          .before = 3 ), digits = 0))


## Count cases by age group and week
ajs_poc_cl_agegrp <- ajs_poc_cl %>% 
  group_by(age_group, epiweek2) %>% 
  ## count number of cases per year and week
  tally() %>% 
  ## average the weekly values of ajs cases for previous 1 month
  mutate(four_week_roll = round(slide_dbl(n, ~mean(.x, na.rm = TRUE), 
...
## Count cases per week per age group for 2019 and 2020
measles_poc_cl_age_grp  <- measles_poc_cl %>% 
  ## only keep measles cases from 2019 and 2020
  filter(year >=2019) %>% 
  ## group date by year and week and age group
  group_by(year, epiweek, age_group) %>% 
  ## county cases by year and week and age group
  tally() 


## count cases per week by outcome status 2019 and 2020
measles_poc_cl_outcome_grp <- measles_poc_cl %>% 
  ## only keep measles cases from 2019 and 2020
  filter(year >=2019) %>% 
  ## group date by year and week and outcome2
  group_by(year, epiweek, outcome2) %>% 
  ## county cases by year and week and outcome2
  tally() 
...
* COVID-19  
  - A total of `r nrow(covid_data_cl %>% filter(msf_covid_status2 == "suspected"))` suspected and `r nrow(filter(covid_data_cl, msf_covid_status2 == "confirmed"))` confirmed cases have been reported in Bentiu PoC, Lankien and Leer (`r nrow(covid_data_cl %>% filter(msf_covid_status2 == "confirmed", site  == "lankien"))` confirmed cases in Lankien,  `r nrow(covid_data_cl %>% filter(msf_covid_status2 == "confirmed", site  == "bentiu"))` in Bentiu PoC and `r nrow(covid_data_cl %>% filter(msf_covid_status2 == "confirmed", site  == "leer"))` in Leer ). `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "lankien"))` deaths in Lankien,  `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "bentiu"))` in Bentiu and `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "leer"))` in Leer. In `r two_weeks_earlier` and `r reporting_week`, there were a total of `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier))` suspected cases, of which `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, msf_covid_status2 == "confirmed"))` were confirmed cases.  `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, msf_covid_status2 %in% c ("suspected", "confirmed"), site == "lankien"))` in Lankien and `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, msf_covid_status2 %in% c ("suspected", "confirmed", "not a case"), site == "bentiu"))` in Bentiu and `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, msf_covid_status2 %in% c ("suspected", "confirmed"), site == "leer"))` in Leer. There were `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, outcome_patcourse_status == "died"))` deaths in the previous two weeks across the projects.  No further suspect cases have been dentified in the Mundri.
  
* Mortality surveillance  
  - Rolling four weekly average number of deaths is stable at `r bentiu_last_current %>% filter(epiweek == reporting_week) %>% pull(four_week_roll)` per week in Bentiu PoC. The Bentiu PoC crude mortality rates per 10,000 population per day over the past four weeks ranged from 
`r bentiu_last_current %>% filter(epiweek >= four_weeks_earlier & epiweek <= reporting_week) %>% summarise(min = round(min(death_rate_total), digits = 2),max = round(max(death_rate_total), digits = 2)) %>% mutate(combo = paste0(min, "-", max)) %>% pull(combo)`. There were a total of `r bentiu_last_current %>% filter(epiweek >= start_last_year, epiweek <= reporting_week_last_year) %>% summarise(count_deaths = sum(total_deaths)) %>% pull(count_deaths)` deaths in Bentiu PoC by `r reporting_week_last_year` compared to `r bentiu_last_current %>% filter(epiweek >= start_year, epiweek <= reporting_week) %>% summarise(count_deaths = sum(total_deaths)) %>% pull(count_deaths)` by `r reporting_week`. Death due to respiratory failure also stable at `r combined_resp_failure %>% filter(epiweek == reporting_week, site == "bentiu") %>% pull(ma_resp_mort_4week)` per week in Bentiu PoC. In Lankien, there have been `r lankien_resp_mort_cl %>% filter(epiweek >= start_last_year, epiweek <= yearweek("2019 W35")) %>% summarise(count_resp = sum(resp_failure)) %>% pull(count_resp)` respiratory failure deaths by 2019 W35 compared to `r lankien_resp_mort_cl %>% filter(epiweek >= start_year, epiweek <= yearweek("2020 W35")) %>% summarise(count_resp = sum(resp_failure)) %>% pull(count_resp)` respiratory failure deaths by 2020 W35 this year.  

* ITFC  
  - In `r reporting_week`, rolling four week average of `r filter(ben_lan_itfc_ma, epiweek == reporting_week, site == "lankien") %>%  pull(ma_itfc_4wk)` in Lankien (`r filter(ben_lan_itfc_ma, epiweek == reporting_week, site == "lankien") %>%  pull(itfc_total_admissions)` admissions in `r reporting_week`) and `r filter(ben_lan_itfc_ma, epiweek == reporting_week, site == "bentiu") %>%  pull(ma_itfc_4wk)` in Bentiu PoC ( `r filter(ben_lan_itfc_ma, epiweek == reporting_week, site == "bentiu") %>%  pull(itfc_total_admissions)` admissions in `r reporting_week`).  
  
* AWD  
  - **AWD cases continue to decline in Lankien, Bentiu PoC and Unity. Cases are stable in Mundri**.  

* Malaria  
  - `r filter(ben_lan_awd_mal, epiweek == reporting_week, site == "bentiu") %>% pull(opd_malaria)` confirmed malaria cases in Bentiu PoC OPD  and `r filter(ben_lan_awd_mal, epiweek == reporting_week, site == "bentiu") %>% pull(iccm_malaria)` in the malaria point in `r reporting_week`. The four-week moving average in `r reporting_week` is `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "bentiu") %>% pull(ma_malaria_4wk)` cases when including OPD and malaria point data.  In `r four_weeks_earlier`, the four-week moving average was `r ssudan_malaria_4ma %>% filter(epiweek == four_weeks_earlier, site == "bentiu") %>% pull(ma_malaria_4wk)` in Bentiu PoC. The four week moving average was `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "lankien") %>% pull(ma_malaria_4wk)`, `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "mundri") %>% pull(ma_malaria_4wk)`, and `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "unity") %>% pull(ma_malaria_4wk)` in  Lankien, Mundri and Unity projects respectively in `r reporting_week`. 
  
* AJS  
  - In `r reporting_week`, the four week average was `r filter(ajs_poc_cl_grp, epiweek2 == reporting_week) %>% pull(four_week_roll)` cases in Bentiu PoC and the cases in the last 21 days were scattered across the camp.
  
* Measles  
  - No new cases of measles in any projects in the past two weeks.  

* Consultation numbers  
  - The number of consultations in Lankien and Mundri follow the trends lines of the previous year but lower overall. Bentiu OPD consultations have returned to lower levels but still large numbers at the malaria points. No significant changes in consultations numbers seen in Unity projects due to COVID-19 outbreak.

* Population estimate  
  - June 2020 population estimate was 99,052 individuals in Bentiu PoC (a drop from 117,766 in May), of which 22% were children aged <5 years. Still waiting on the July/August population update. 
  

## COVID-19
There are currently a  total of `r nrow(filter(covid_data_cl, msf_covid_status2 == "suspected"))` suspected  and `r nrow(filter(covid_data_cl, msf_covid_status2 == "confirmed"))` confirmed COVID-19 cases in Bentiu, Lankien and Leer reported (excluding non-cases). `r nrow(filter(covid_data_cl, msf_covid_status2 == "confirmed", site == "lankien" ))` confirmed cases in Lankien and `r nrow(filter(covid_data_cl, msf_covid_status2 == "confirmed", site == "bentiu"))` in Bentiu Poc and `r nrow(filter(covid_data_cl, msf_covid_status2 == "confirmed", site == "leer"))` in Leer. There have been `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died"))` deaths among suspected and confirmed cases with `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "lankien"))` in Lankien and `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "bentiu"))` in Bentiu and `r nrow(filter(covid_data_cl, outcome_patcourse_status == "died", site == "leer"))` in Leer. There were a total of `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier))` suspected cases identified in Bentiu, Lankien and Leer in `r two_weeks_earlier` and `r reporting_week`, with `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, site == "lankien"))` in Lankien and  `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, site == "bentiu"))` in Bentiu and `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, site == "leer"))` in Leer. There were `r nrow(filter(covid_data_cl, epiweek == reporting_week | epiweek == two_weeks_earlier, outcome_patcourse_status == "died"))` deaths reported during `r two_weeks_earlier` and `r reporting_week`. No further suspect cases identified in Mundri beyond the initial two cases identified at Mundri PHCC to date.
...
```{r combine_case_age_epicurves, fig.width = 14, fig.height = 6}
...
```{r covid_age_sex_pyramids}

## Benitu PoC age-sex pyramid
age_sex_bentiu  <- plot_age_pyramid(filter(covid_data_cl,site == "bentiu"),
                                    age_group = "age_group", 
...
## Lankien age-sex pyramid
age_sex_lankien  <- plot_age_pyramid(filter(covid_data_cl,site == "lankien"),
                                    age_group = "age_group", 
                                    split_by = "patinfo_sex",
                                    na.rm = TRUE) +  # exclude missing values
  labs(y = "Cases (n)", x = "Age group") + # change axis  labels
...
## Leer age-sex pyramid
age_sex_leer  <- plot_age_pyramid(filter(covid_data_cl,site == "leer"),
                                    age_group = "age_group", 
                                    split_by = "patinfo_sex",
                                  na.rm = TRUE) +  # exclude missing values
  labs(y = "Cases (n)", x = "Age group") + # change axis  labels
...
<!-- #### Combine age sex pyramids -->

```{r combine_covid_age_sex_pyramids, fig.width = 14, fig.height = 7}

age_sex_bentiu + age_sex_lankien + age_sex_leer

```

The below table provides additional information on the deaths among suspected and confirmed COVID-19 cases by project location. 

<!-- <!-- #### Characteristics of dead cases by age group and site --> 
...
## examine only dead cases 
 ...
 tab_linelist(covid_deaths, age_group, strata = "site") %>% 
   select(-variable) %>% 
...
### Mortality surveillance
In Bentiu PoC, the current four-week moving average is `r bentiu_last_current %>% filter(epiweek == reporting_week) %>% pull(four_week_roll)` cases per week. The crude mortality rate for the total population between `r four_weeks_earlier` and `r reporting_week` was `r bentiu_last_current %>% filter(epiweek >= four_weeks_earlier, epiweek <= reporting_week) %>% select(death_rate_total) %>% mutate(death_rate_total = round(death_rate_total, digits = 2)) %>% summarise(range = range(death_rate_total)) %>% pull(range)` deaths/10,000 population/day.
...
## Respiratory failure
**THIS DATA has not been updated from the previous SitRep**

The total number of deaths due to respiratory failure in Bentiu PoC up to 2020 W35 is `r combined_resp_failure %>% filter(epiweek >= start_year, epiweek <= reporting_week, site == "bentiu") %>% summarise(total_deaths = sum(resp_failure)) %>% pull(total_deaths)` compared to `r combined_resp_failure %>% filter(epiweek >= start_last_year, epiweek <= yearweek("2019 W35"), site == "bentiu") %>% summarise(total_deaths = sum(resp_failure)) %>% pull(total_deaths)` in the same period in the previous year. Similar figures for Lankien are `r combined_resp_failure %>% filter(epiweek >= start_year, epiweek <= reporting_week, site == "lankien") %>% summarise(total_deaths = sum(resp_failure)) %>% pull(total_deaths)` for `r reporting_year` and `r combined_resp_failure %>% filter(epiweek >= start_last_year, epiweek <= yearweek("2019 W35"), site == "lankien") %>% summarise(total_deaths = sum(resp_failure)) %>% pull(total_deaths)` for `r reporting_last_year`. However, not all deaths due to respiratory failure had COVID-19 like symptoms as their primary/secondary diagnoses on admission.
...
## Nutrition
In `r reporting_week`, the four week rolling average is `r ben_lan_itfc_ma %>% filter(site == "bentiu", epiweek == reporting_week) %>% pull(ma_itfc_4wk)` cases/week in Bentiu PoC and `r ben_lan_itfc_ma %>% filter(site == "lankien", epiweek == reporting_week) %>% pull(ma_itfc_4wk)` in  Lankien. There were `r ben_lan_itfc_ma %>% filter(site == "bentiu", epiweek == reporting_week) %>% pull(itfc_total_admissions)` ITFC admissions in Bentiu  and `r ben_lan_itfc_ma %>% filter(site == "lankien", epiweek == reporting_week) %>% pull(itfc_total_admissions)` in Lankien in `r reporting_week`. 

```{r itfc_epicurve, fig.height = 6}
...
The proportion of Severe Acute Malnutrition (SAM) cases in Unity and Mundri OPDs were beneath the emergency threshold (<2% indicating generally food secure).

```{r plot_unity_mundri_nutrition, fig.height = 6}

ggplot(unity_mundri_nutrition_raw, 
       aes(x = month, 
           y = sam_proportion)) +
  ...
  theme(axis.text.x = element_text(angle = 90 )) +
  ## Add labels
  labs(title = "Proportion of SAM in Unity and Mundri, 2019-2020",
       x = "Month",
       y = "Proportion of SAM")
...
## AWD 
CBMC data and OPD data are now included in these graphs and figures. In `r reporting_week`, there were `r ssudan_awd_4ma %>% filter(site == "bentiu", epiweek == reporting_week) %>% pull(all_awd)` cases of AWD in  Bentiu PoC, `r ssudan_awd_4ma %>% filter(site == "lankien", epiweek == reporting_week) %>% pull(all_awd)` cases in Lankien, `r ssudan_awd_4ma %>% filter(site == "unity", epiweek == reporting_week) %>% pull(all_awd)` cases in Unity and `r ssudan_awd_4ma %>% filter(site == "mundri", epiweek == reporting_week) %>% pull(all_awd)` cases in Mundri. The four week average were `r ssudan_awd_4ma %>% filter(site == "bentiu", epiweek == reporting_week) %>% pull(ma_awd_4wk)`, `r ssudan_awd_4ma %>% filter(site == "lankien", epiweek == reporting_week) %>% pull(ma_awd_4wk)`,`r ssudan_awd_4ma %>% filter(site == "mundri", epiweek == reporting_week) %>% pull(ma_awd_4wk)` and `r ssudan_awd_4ma %>% filter(site == "unity", epiweek == reporting_week) %>% pull(ma_awd_4wk)` in Bentiu, Lankien, Mundri and Unity respectively.

<!-- ## all projects -->
```{r awd_all_epicurve, fig.width = 16, fig.height = 8}
...
ggplot() +
  geom_col(data = ssudan_awd_wide, 
           aes(x = epiweek, 
               y = count, 
               fill = factor(awd_age,
                             levels = c("all_awd_under5", "all_awd_over5"),
  ...
  ## rotate x axis text
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Weekly AWD cases in Bentiu PoC, Lankien, Mundri and Unity with a four-week rolling average, 2019-2020",
       x = "Month",
...
## Malaria
CBMC data and OPD data are now included in these graphs and figures. The current four-week moving average is `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "bentiu") %>% pull(ma_malaria_4wk)` cases per week in Bentiu, `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "lankien") %>% pull(ma_malaria_4wk)` in Lankien, `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "mundri") %>% pull(ma_malaria_4wk)` in Mundri and `r ssudan_malaria_4ma %>% filter(epiweek == reporting_week, site == "unity") %>% pull(ma_malaria_4wk)` in Unity. In `r four_weeks_earlier`, the four-week moving average was `r ssudan_malaria_4ma %>% filter(epiweek == four_weeks_earlier, site == "bentiu") %>% pull(ma_malaria_4wk)` in Bentiu PoC.

<!-- #### Epicurve all projects -->
```{r malaria_all_epicurve, fig.width = 16, fig.height = 8}
## Histogram with line with 4 weekly average
ggplot() +
  geom_col(data = ssudan_malaria_wide, 
           aes(x = epiweek, 
               ...
               fill = factor(malaria_age,
                             levels = c("all_malaria_under5", "all_malaria_over5"),
                             labels = c("<5 yrs", "5+ yrs")))) +
  ## add four week moving \naverage
...
The last case of AJS in Lankien was reported in `r ben_lan_awd_mal %>% filter(site == "lankien") %>% rowwise() %>%  mutate(any_ajs = sum(acute_jaundice_syndrome_ages_5y, acute_jaundice_syndrome_ages_5y_2, na.rm = TRUE)) %>%  filter(any_ajs >=1) %>% select(epiweek, any_ajs, site) %>% mutate(date_week = as.Date(epiweek, format = "%y-%m-%d")) %>% group_by(site) %>% summarize(max_date = max(date_week)) %>% mutate(max_date_week = yearweek(max_date)) %>%  pull(max_date_week)`
...
## AJS epicurve
## Plot weeks vs AJS cases by age
ajs_ben_epicurve_count <- ggplot(ajs_poc_cl_grp, aes(x = epiweek2, 
                           y = n, 
                                fill = as.factor(year))) +
  geom_col( ) +
  ## add the four week average as a line
...
<!-- ### AJS epicurve by age -->
```{r bentiu_ajs_epicurve_age}

## AJS epicurve
## Plot weeks vs AJS cases by year
ajs_ben_epicurve_age <- ggplot(ajs_poc_cl_agegrp, aes(x = epiweek2, 
                           y = n, 
                                fill = as.factor(age_group))) +
  geom_col( ) +
  # ## add the four week average as a line
...
ajs_ben_epicurve_outcome <- ggplot(ajs_poc_cl_outcome, aes(x = epiweek2, 
                           y = n, 
  ...
  theme(axis.text.x = element_text(angle = 90)) +
  ## Add labels
  labs(title = "Number of weekly AJS cases in Bentiu PoC by \noutcome status, 2019-2020",
       x = "Week",
       y = "Number of AJS cases",
       fill = "Outcome",
       colour = "")
...
<!-- #### Combine AJS epicurves -->
```{r combine_ajs_epicurves, fig.width = 16, fig.height = 7}

ajs_ben_epicurve_age + ajs_ben_epicurve_outcome 
...
```{r choropleth_ajs_past3weeks, message = FALSE, warning = FALSE, echo = FALSE, error = FALSE}
...
```{r choropleth_ajs_2020, message = FALSE, warning = FALSE, echo = FALSE, error= FALSE}
...
```{r combine_ajs_maps, fig.width = 16, fig.height = 7}
...
## Measles
From the start of `r reporting_year` until `r reporting_week` there were a 
total of `r nrow(filter(measles_poc_cl, year == 2020 ))` suspected measles cases and `r nrow(filter(measles_poc_cl, year == 2020, outcome2 == "dead"))` suspected measles deaths in Bentiu PoC. There were
...
There were a total of `r nrow(filter(measles_poc_cl, epiweek == reporting_week))` cases in `r reporting_week`. The most affected age group since the start of `r reporting_year` was `r tab_linelist(filter(measles_poc_cl, year == 2020), age_group) %>% slice(which.max(n)) %>% pull(value)` years.

The last case of measles in Lankien was on `r ben_lan_awd_mal %>% filter(is.na(measles_ages_5y) == F | is.na(measles_ages_5y_2) == F, site == "lankien") %>% mutate(date_week = as.Date(epiweek, format = "%y-%m-%d")) %>% group_by(site) %>% summarize(max_date = max(date_week)) %>% mutate(max_date_week = yearweek(max_date)) %>%  pull(max_date_week)`.
...
## Plot weeks vs age group by year
...
<!-- ## Measles POC epicurve by outcome -->


```{r measles_poc_outcome_epicurve}
## Plot weeks vs deaths by year
measles_bentiu_outcome_epicurve <- ggplot() +
  geom_col(data = measles_poc_cl_outcome_grp, 
           aes(x = epiweek,
               y = n, 
               fill = as.factor(outcome2))) +
  ## add the four week average as a line
  geom_line(data = measles_poc_cl_grp,
            aes(x = epiweek, 
                y = four_week_roll,
  ...
  scale_colour_manual(values = c("moving \naverage" = "black")) +
  ## Add labels
  labs(title = "Weekly measles cases in Bentiu PoC by \n outcome status and with a four-week rolling average, 2019-2020",
       x = "Month",
       y = "Number of measles cases",
       fill = "Outcome status",
       colour = "")
...
```{r combine_measles_epicurves, fig.width = 16, fig.height = 7}

measles_bentiu_age_epicurve + measles_bentiu_outcome_epicurve
...
```{r combine_measles_maps, fig.width = 16, fig.height = 7}
...
## Limitations  
* Data may not be received from some health sites at the time the data was compiled  

* Surveillance data are not representative of the entire population  

* Trends may be impacted in changes to health service provision i.e. additional malaria points opened  

*  The lack of population data makes it difficult to assess the incidence of diseases across projects and only permits us to compare case numbers  

* Due to delays in processing of lab samples and difficulty in accessing Lankien, we may not see an increase in the number of confirmed COVID-19 cases in the near future    


## Actions/updates
* The MoH has revised the COVID-19 case defintion and it now no longer includes an epidemiological component. By following these criteria across all projects, we will see a large increase in suspect cases. This will likely be very challenging given our current COVID-19 bed capacity and the delays getting lab results

* The MoH has revised the case investigation form in line with the revised case definition. All projects will need to start using the revised case investigation form moving forward.

*  Bentiu PoC has implemented changes to screening and testing and is starting to see an increase in suspected cases (all have tested negative so far) This will likely further increase as routine testing of RTI cases in the ITFC is fully implemented

* COVID-19 linelist is available in Mundri and suspect cases will be added to it moving forward

* A webinar on the COVID-19 patient card will take place tomorrow and should help clarify why and how it should be used

* Indoor residual spraying in Bentiu PoC should have taken place last weekend.

* Draft of a CBMC data analysis available for Lankien (2019-2020 data). It will be shared in the coming days and analysis of Mundri CBMC data will be next followed by Unity.

* IPD and OPD data for Lankien (2014-2020) have been obtained to conduct time-series analysis. This analysis will be led by the epi advisor and shared in the coming weeks.

* DHIS2 implementation plans for Mundri/Unity still ongoing but hopefully before the end of the year in both sites.
...
## Annex
The majority of cases that are classified as “other” in the below figure were sent to the CTC = COVID treatment centre (also the cholera treatment centre during cholera outbreaks.
...
ggplot(covid_outcome_counts, aes(x = epiweek, 
  ...
  theme(axis.text.x = element_text(angle = 90)) +
  ## Add labels
  labs(title = "Number of weekly suspected and confirmed cases\n by outcome status in Bentiu PoC, Lankien and Leer, 2020",
       x = "Week",
       y = "Number of cases",
       fill = "Outcome status") +
  theme(legend.title = element_text(size =14))
...
<!-- ## Plot of Bentiu and Lankien consultations -->
The OPD consultations in Bentiu PoC returning to pre-malaria outbreak level but still very large numbers of consultations at the malaria point (data not shown here). OPD consultations in Lankien remain at lower levels compared to the same period last year.

```{r plot_ben_lan_opd_consultations}

## Plot number of OPD consultations over time in Bentiu and Lankien
ggplot(ben_lan_awd_mal, 
       aes(x = epiweek, y = total_opd_consultations)) +
  geom_col(colour = "blue") +
  ## adjust the ticks on x-axis
  scale_x_yearweek(date_breaks = "3 months", date_labels = "%U-%Y") +
  # facet by site
  facet_grid( . ~ site) +
  # Adjust size of plot title
  theme(plot.title = element_text(size = 14)) +
  ...
  theme(axis.text.x = element_text(angle = 90)) +
  ## Add labels
  labs(title = "Number of weekly OPD consultations in Bentiu PoC and Lankien, 2019-2020",
       x = "Week",
       y = "Number of cases") +
  theme(legend.title = element_text(size =14))
