...
ben_lan_mal_ma <- ben_lan_awd_mal %>%
  ## add 4 moving averages for malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek, key = site) %>%
  group_by(site) %>%
  ## create a moving average variable (deals with missings)
...
zero_report <- data.frame(year = c(2020,2020,2020,2020),
...
ggplot(bentiu_last_current, aes(x = epiweek, 
  ...
  geom_line(aes(x = epiweek, y = four_week_roll, colour = "moving average"), 
            size = 1.2) +
  # adjus the x axis
...
ggplot(bentiu_resp_mort_cl, aes(x = epiweek, 
  ...
  geom_col() +
  ## add 4 week moving average
  geom_line(aes(x = epiweek, y = ma_resp_mort_4week, colour = "moving average"),
            size = 1.2) +
...
### Malaria
The current four-week moving average is `r ben_lan_awd_mal_ma %>% filter(epiweek == reporting_week, site == "bentiu") %>% pull(ma_malaria_4wk)` cases per week in Bentiu and `r ben_lan_awd_mal_ma %>% filter(epiweek == reporting_week, site == "lankien") %>% pull(ma_malaria_4wk)` cases per week in Lankien. `r four_weeks_earlier`, the four-week moving average was `r ben_lan_awd_mal_ma %>% filter(epiweek == four_weeks_earlier, site == "bentiu") %>% pull(ma_malaria_4wk)` in Bentiu PoC in `r four_weeks_earlier`

#### Epicurve all projects
```{r malaria_all_epicurve}
## Histogram with line with 4 weekly average
ggplot() +
  ...
  geom_line(data = ssudan_malaria_4ma,
            aes(x = epiweek, 
                y = ma_malaria_4wk, colour = "Moving average"),
            size = 1.2)  + 
  ## adjust the date scale on the x axis
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ##create a legend for the 4 week moving average
  ...
  ## rotate x axis text
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
  labs(title = "Weekly malaria cases in Bentiu PoC, Lankien, Mundri and Unity with a four-week rolling average, 2019-2020",
       x = "Month",
       y = "Number of AWD cases",
       fill = "Age group",
       colour = "")
...
ggplot(ajs_poc_cl_grp, aes(x = epiweek2, 
  ...
  geom_line(aes(x = epiweek2, 
                y = four_week_roll, colour = "moving average"), 
            size = 1.2) +
