...
## set current week 
reporting_week <- aweek::as.aweek("2020-W29")

## set three weeks earlier
three_weeks_earlier <- aweek::as.aweek("2020-W27")


## set start of the year
start_year <- as.aweek("2020-W01")
...
## Bentiu PoC
## 2019
bentiu_mort_2019_raw <- rio::import(here::here("7_data",
                                               "mortality", 
                                               "raw",
...
bentiu_mort_current_raw <- rio::import(here::here("7_data",
                                                  "mortality", 
                                                  "raw",
...
## AJS  ----------------------------------------------------
ajs_poc_raw <- rio::import(here::here("7_data", 
                                      "ajs",
                                      "raw", 
...
bentiu_last_current <- bentiu_last_current %>% 
  ## create aweek compatible epiweek variable
   mutate(epiweek = if_else(week <10, str_pad(week, 2, pad = 0), as.character(week)),
         epiweek = as.aweek(paste0(year, "-W",epiweek))) %>% 
...
ajs_poc_clean <- ajs_poc_raw %>%
  ...
  mutate(epiweek2 = if_else(epi_week <10, str_pad(epi_week, 2, pad = 0), as.character(epi_week)),
         epiweek2 = as.aweek(paste0(year, "-W",epiweek2))) %>% 
...
## Count cases by year and week
ajs_poc_clean_grp <- ajs_poc_clean %>% 
...
dictionary <- rio::import(here::here("7_data", "ajs", "dictionary_ssudan.xlsx"))

## This step fixes the values so that you can read them.
## values like 1/0 will be recoded as "Yes" / "No" based on the dictionary
 ajs_poc_clean <- matchmaker::match_df(ajs_poc_clean,
  dict  = dictionary,
  from  = "options",
  to    = "values",
  by    = "grp",
...
ggplot(bentiu_last_current, aes(x = week2date(epiweek), y = total_deaths, 
                                fill = as.factor(year))) +
  geom_col( ) +
  ## add the four week average as a line
  geom_line(aes(x = week2date(epiweek), y = four_week_roll), 
...
total of `r nrow(filter(ajs_poc_clean, year == 2020)`. There were
`r fmt_count(filter(ajs_poc_clean, year == 2020), sex == "Female")` females and
`r fmt_count(filter(ajs_poc_clean, year == 2020), sex == "Male")` males. 
There were a total of `r nrow(filter(ajs_poc_clean, epiweek2 == reporting_week))` in `r reporting_week`

The most affected age group was `r tab_linelist(filter(ajs_poc_clean, year == 2020), age_group) %>% slice(which.max(n)) %>% pull(value)` years. 
...
ggplot(ajs_poc_clean_grp, aes(x = week2date(epiweek2), y = n, 
                                fill = as.factor(year))) +
  geom_col( ) +
  ## add the four week average as a line
  geom_line(aes(x = week2date(epiweek2), y = four_week_roll), 
            size = 1.2) +
