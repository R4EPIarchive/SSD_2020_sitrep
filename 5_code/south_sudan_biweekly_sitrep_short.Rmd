...
## set current week 
reporting_week <- yearweek("2020 W29")

## set previous week
two_weeks_earlier <- yearweek("2020 W28")

## set three weeks earlier
three_weeks_earlier <- yearweek("2020 W27")

## set four weeks earlier
four_weeks_earlier <- yearweek("2020 W26")
...
## AWD/malaria ------------------------------------------------------------------
### bentiu
bentiu_awd_mal_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "bentiu",
                                          "bentiu_awd_malaria_week29.xls"), 
                              skip = 2,
...
###lankien
lankien_awd_mal_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "lankien",
                                          "lankien_awd_malaria_week29.xls"), 
                              skip = 2,
...
## Mundri AWD
mundri_awd_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "mundri",
                                          "mundri_awd_week29.xlsx"), 
                              which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## ITFC  ------------------------------------------------------------------
### bentiu
bentiu_itfc_raw <- rio::import(here::here("6_data",
                                         "itfc", 
                                          "bentiu",
                                          "weekly_itfc_week29.xlsx"), 
                              which = "Sheet1")  %>% 
  ## make all variable names lower case
  janitor::clean_names()

###lankien
lankien_itfc_raw <- rio::import(here::here("6_data",
                                         "itfc", 
                                          "lankien",
                                          "lankien_weekly_itfc_week29.csv"), 
                              which = "lankien_weekly_itfc_week29") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Respiratory failure  ------------------------------------------------------------------
### bentiu
bentiu_resp_mort_raw <- rio::import(here::here("6_data",
                                         "resp_failure", 
                                          "resp_failure_bentiu_week29.xls"), 
                              skip = 2,
                              which = "Bentiu Healthcare")  %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## COVID data ------------------------------------------------------------------
## Bentiu
bentiu_covid_raw  <- rio::import(here::here("6_data",
                                             "covid", 
                                             "raw",
                                             "linelist_Covid_Bentiu PoC_2020-07-21.xlsx"),
                                  which = "linelist")
                                   
## Lankien
lankien_covid_raw  <- rio::import(here::here("6_data",
                                             "covid", 
                                             "raw",                                      "linelist_Covid_Lankien_2020-07-20.xlsx"),
                                  which = "linelist")                                   
...
## Bentiu PoC
## 2019
bentiu_mort_2019_raw <- rio::import(here::here("6_data",
                                               "mortality", 
                                               "raw",
                                               "Bentiu PoC Mortality Line List Report for IDP Sites Wk 52-2019.xls"),
                                    skip = 2, 
...
bentiu_mort_current_raw <- rio::import(here::here("6_data",
                                                  "mortality", 
                                                  "raw",
                                                  "Bentiu PoC Mortality Line List Report for IDP Sites Wk 29.xls"), 
                                       skip = 2,
                                       which = "Mortality rate") %>% 
  ## make all variable names lower case
  janitor::clean_names() %>% 
...
## Bentiu POC AJS  ----------------------------------------------------
ajs_poc_raw <- rio::import(here::here("6_data", 
                                      "ajs",
                                      "raw", 
                                      "MSF_Bentiu PoCs_AJS line list_Wk 29- 2020.xlsx"),
                           skip = 3,
                           which = "AJS") %>% 
  ## make all variable names lower case
  janitor::clean_names()  %>% 
...
measles_poc_raw <- rio::import(here::here("6_data", 
                                      "measles",
                                      "raw", 
                                      "MSF_Bentiu_Measles Line List_Wk 29-2020.xlsx"),
...
## Unity AWD
### 2019
unity_awd_2019_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "unity",
                                          "unity_awd_2019.xlsx"), 
                              which = "Sheet1") %>% 
...
## 2020
unity_awd_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "unity",
                                          "unity_awd_week29.xlsx"), 
                              which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Mundri AWD
### 2019
mundri_awd_2019_raw <- rio::import(here::here("6_data",
                                         "awd_malaria",
                                          "mundri",
                                          "mundri_awd_2019.xlsx"),
                              which = "Sheet1") %>%
  ## make all variable names lower case
  janitor::clean_names()
...
## Unity Malaria
### 2019
unity_malaria_2019_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "unity",
                                          "unity_malaria_2019.xlsx"), 
                              which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## 2020
unity_malaria_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "unity",
                                          "unity_malaria_week29.xlsx"), 
                              which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Mundri malaria
### 2019
mundri_mal_2019_raw <- rio::import(here::here("6_data",
                                         "awd_malaria",
                                          "mundri",
                                          "mundri_malaria_2019.xlsx"),
                              which = "Sheet1") %>%
  ## make all variable names lower case
  janitor::clean_names()
...
## 2020
mundri_mal_raw <- rio::import(here::here("6_data",
                                         "awd_malaria", 
                                          "mundri",
                                          "mundri_malaria_week29.xlsx"), 
                              which = "Sheet1") %>% 
  ## make all variable names lower case
  janitor::clean_names()
...
## Cleaning DHIS2 data
### AWD and malaria Bentiu and Lankien
- Add a site variable to both datasets
- combine both datasets
- select awd and malaria variables
- add under5 and over 5 data
- create a tsibble object so that can create a 4 week rolling average
- create a 4 week rolling average for awd and malaria
...
ben_lan_awd_mal <- bind_rows(bentiu_awd_mal_raw, lankien_awd_mal_raw) %>% 
  ...
  mutate(all_awd = awd_under5 + awd_over5,
         all_malaria = malaria_under5 + malaria_over5) %>% 
...
ben_lan_awd_mal_ma <- ben_lan_awd_mal %>% 
  ...
  mutate(ma_awd_4wk = slide_dbl(all_awd, ~mean(.x, na.rm = TRUE), .before = 3 ),
         ma_mal_4wk = slide_dbl(all_malaria, ~mean(.x, na.rm = TRUE), .before = 3 ))
...
bentiu_resp_mort_cl <- bentiu_resp_mort_raw %>% 
  ...
  mutate(ma_resp_mort_4week = rollmean(resp_failure, 4,
                                       na.pad = TRUE,
                                       align = "right"))
...
ben_lan_itfc_ma <- ben_lan_itfc_cl %>% 
  ...
  group_by(site) %>% 
  ## create a moving average variable (deals with missings)
  mutate(ma_itfc_4wk = slide_dbl(number, ~mean(.x, na.rm = TRUE), .before = 3 ))
...
unity_awd_19_20_wide <- unity_awd_19_20_cl %>% 
  ## select the variables to keep
  select(epiweek2, site, under5, over5) %>% 
  pivot_longer(cols = c("under5", "over5"),
               names_to = "awd_age",
               values_to = "count")
...
unity_awd_19_20_ma <- unity_awd_19_20_cl %>% 
  ## add 4 moving averages for awd and malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek2, key = site) %>% 
  group_by(site) %>% 
  ## create a moving average variable (deals with missings)
  mutate(ma_awd_4wk = slide_dbl(all_awd, ~mean(.x, na.rm = TRUE), .before = 3 ))
...
## Create dataset with moving averages
mundri_awd_19_20_cl_ma <- mundri_awd_19_20_cl %>% 
  ## add 4 moving averages for awd and malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek2, key = site) %>% 
  group_by(site) %>% 
...
unity_malaria_19_20_cl <- bind_rows(unity_malaria_2019_cl, unity_malaria_cl) %>% 
  ## add total value of awd cases
  mutate(all_malaria = under5 + over5)
...
unity_malaria_19_20_ma <- unity_malaria_19_20_cl %>% 
  ## add 4 moving averages for malaria and malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek2, key = site) %>% 
  group_by(site) %>% 
  ## create a moving average variable (deals with missings)
...
## Create dataset with moving averages
mundri_malaria_19_20_cl_ma <- mundri_malaria_19_20_cl %>% 
  ## add 4 moving averages for malaria and malaria
  ## make counts in to tsibble so it recognises weeks as main time var
  as_tsibble(index = epiweek2, key = site) %>% 
  group_by(site) %>% 
...
covid_case_counts <- covid_data_cl %>% 
  ...
  group_by(epiweek, site, msf_covid_status) %>% 
  ## tally
  tally()
...
covid_outcome_counts <- covid_data_cl %>% 
  ## group by week, site, status
  group_by(epiweek, site, outcome_patcourse_status) %>% 
  ## tally
  tally()
...
bentiu_last_current <- bentiu_last_current %>% 
  ...
  ## average the weekly values of deaths for previous 4 weeks
  mutate(four_week_roll = rollmean(total_deaths, 4, na.pad = TRUE, align = "right"))

```


## Cleaning of AJS data
...
ajs_poc_cl_grp <- ajs_poc_cl %>% 
  ...
  tally() %>% 
  ## average the weekly values of ajs cases for previous 4 weeks
  mutate(four_week_roll = rollmean(n, 4, na.pad = TRUE, align = "right"))
...
Second step
- create a zero report dataset for weeks 28 and 29, which can be updated as needed
...
zero_report <- data.frame(year = c(2020,2020),
                          epiweek = yearweek(c("2020 W28", "2020 W29")),
                          n = c(0,0))
...
ggplot(covid_case_counts, aes(x = epiweek, 
                              y = n, 
                              fill = as.factor(msf_covid_status))) +
  geom_col( ) +
  ## adjust the ticks on x-axis
  scale_x_yearweek(date_breaks = "3 weeks", date_labels = "%U-%Y") +
  # facet by site
  facet_grid( . ~ site) +
  ...
  ## Add labels
  labs(title = "Number of weekly suspected and confirmed COVID-19 cases\n in Bentiu PoC and Lankien, 2020",
       x = "Week",
       y = "Number of cases",
...
ggplot(covid_outcome_counts, aes(x = epiweek, 
                                 y = n, 
                                 fill = as.factor(outcome_patcourse_status))) +
  geom_col( ) +
   ## adjust the ticks on x-axis
  scale_x_yearweek(date_breaks = "3 weeks", date_labels = "%U-%Y") +
  # facet by site
...
ggplot(covid_age_counts, aes(x = epiweek,
                             y = n, 
...
for (i in c("bentiu", "lankien")) {
 ...
 age_sex <- plot_age_pyramid(filter(covid_data_cl,site == i),
  ...
  theme(legend.position = "bottom",     # move legend to bottom
   ...
   ggtitle(paste0("Age-sex pyramid of suspected and confirmed COVID-19 cases in ", i))
...
ggplot(bentiu_last_current, aes(x = epiweek, 
  ...
  geom_line(aes(x = epiweek, y = four_week_roll), 
...
## Respiratory failure
### Respiratory failure epicurve

```{r resp_failure_epicurve}

ggplot(bentiu_resp_mort_cl, aes(x = epiweek, 
                                y = resp_failure,
                                fill = as.factor(year))) +
  geom_col() +
  ## add 4 week moving average
  geom_line(aes(x = epiweek, y = ma_resp_mort_4week),
            size = 1.2, colour = "black") +
  ## adjust x axis breaks
  scale_x_yearweek(date_breaks = "2 month", date_labels = "%m-%y") +
  ## Add labels
  theme(plot.title = element_text(size = 16)) +
  labs(title = "Number of weekly respiratory deaths in Bentiu PoC \nwith a four-week rolling average, 2019-2020",
       x = "Month",
...
ggplot(ben_lan_itfc_ma, aes(x = epiweek, 
  ...
  geom_col() +
  ## add 4 week moving average
  geom_line(aes(x = epiweek, y = ma_itfc_4wk),
            size = 1.2, colour = "black") +
  ## adjust x axis breaks
  scale_x_yearweek(date_breaks = "2 month", date_labels = "%m-%y") +
  ## facet by site
...
ggplot() +
  geom_col(data = ben_lan_awd_wide, 
           aes(x = epiweek, 
               y = count, 
               fill = factor(awd_age,
  ...
  ## add four week moving average
  geom_line(data = ben_lan_awd_mal_ma,
            aes(x = epiweek,
                y = ma_awd_4wk),
            size = 1.2, colour = "black")  +
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ...
  ## rotate x axis text
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5)) +
  labs(title = "Weekly AWD cases in Bentiu PoC and Lankien \nwith a four-week rolling average, 2019-2020",
       x = "Month",
...
ggplot() +
  geom_col(data = unity_awd_19_20_wide, aes(x = epiweek2, y = count, 
                                           fill = factor(awd_age,
                             levels = c("under5", "over5"),
                             labels = c("<5 yrs", "5+ yrs")))) +
  ## add in rolling 4 weeks
  geom_line(data = unity_awd_19_20_ma, aes(x = epiweek2, y = ma_awd_4wk),
            size = 1.2, colour = "black") +
  ## adjust the date breaks
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ## adjust the plot title size
  theme(plot.title = element_text(size = 14)) +
  ## Add labels
  labs(title = "Weekly AWD cases in Unity with a four week rolling average, 2019-2020",
       x = "Month",
       y = "Number of AWD cases",
       fill = "Age group")
...
### Mundri AWD
```{r awd_mundri_epicurve}

ggplot() +
  geom_col(data = mundri_awd_19_20_cl_wide, aes(x = epiweek2, y = count, 
                                          fill = factor(awd_age,
                             levels = c("under5", "over5"),
                             labels = c("<5 yrs", "5+ yrs")))) +
  ## add in rolling 4 weeks
  geom_line(data = mundri_awd_19_20_cl_ma, aes(x = epiweek2, y = ma_awd_4wk),
            size = 1.2, colour = "black") +
  ## adjust the date breaks
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ## adjust the plot title size
  theme(plot.title = element_text(size = 14)) +
  ## Add labels
  labs(title = "Weekly AWD cases in Mundri with a \nfour week rolling average, 2019-2020",
       x = "Month",
       y = "Number of AWD cases",
       fill = "Age group")
...
## Malaria
The current four-week moving average is `r ben_lan_awd_mal_ma %>% filter(epiweek == reporting_week, site == "bentiu") %>% pull(ma_mal_4wk)` cases per week in Bentiu and `r ben_lan_awd_mal_ma %>% filter(epiweek == reporting_week, site == "lankien") %>% pull(ma_mal_4wk)` cases per week in Lankien. `r four_weeks_earlier`, the four-week moving average was `r ben_lan_awd_mal_ma %>% filter(epiweek == four_weeks_earlier, site == "bentiu") %>% pull(ma_mal_4wk)`

### Bentiu and Lankien
```{r malaria_ben_lan_epicurve}
## Histogram with line with 4 weekly average
ggplot() +
  geom_col(data = ben_lan_mal_wide, 
           aes(x = epiweek, 
               y = count, 
  ...
  ## Add labels
  theme(plot.title = element_text(size = 14)) +
  labs(title = "Weekly malaria cases in Bentiu PoC and Lankien \n with a four-week rolling average, 2019-2020",
       x = "Month",
...
### Unity malaria
```{r malaria_unity_epicurve}

ggplot() +
  geom_col(data = unity_malaria_19_20_wide, aes(x = epiweek2, y = count, 
                                           fill = factor(malaria_age,
                             levels = c("under5", "over5"),
                             labels = c("<5 yrs", "5+ yrs")))) +
  ## add in rolling 4 weeks
  geom_line(data = unity_malaria_19_20_ma, aes(x = epiweek2, 
                                               y = ma_malaria_4wk),
            size = 1.2, colour = "black") +
  ## adjust the date breaks
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ...
  theme(plot.title = element_text(size = 14)) +
  ## Add labels
  labs(title = "Weekly malaria cases in Unity with a \nfour week rolling average, 2019-2020",
       x = "Month",
       y = "Number of malaria cases",
       fill = "Age group")
...
### Mundri malaria
```{r malaria_mundri_epicurve}

ggplot() +
  geom_col(data = mundri_malaria_19_20_cl_wide, aes(x = epiweek2, y = count, 
                                          fill = factor(malaria_age,
                             levels = c("under5", "over5"),
                             labels = c("<5 yrs", "5+ yrs")))) +
  ## add in rolling 4 weeks
  geom_line(data = mundri_malaria_19_20_cl_ma, aes(x = epiweek2,
                                             y = ma_malaria_4wk),
            size = 1.2, colour = "black") +
  ## adjust the date breaks
  scale_x_yearweek(date_breaks = "3 month", date_labels = "%m-%y") +
  ...
  theme(plot.title = element_text(size = 14)) +
  ## Add labels
  labs(title = "Weekly malaria cases in Mundri with a \nfour week rolling average, 2019-2020",
       x = "Month",
       y = "Number of malaria cases",
       fill = "Age group")
...
`r fmt_count(filter(ajs_poc_cl, year == 2020), sex == "Male")` males. The most affected age group overall in 2020 is `r tab_linelist(filter(ajs_poc_cl, year == 2020), age_group) %>% slice(which.max(n)) %>% pull(value)` years. There were a total of `r nrow(filter(ajs_poc_cl, epiweek2 == reporting_week))` AJS cases in `r reporting_week` and `r nrow(filter(ajs_poc_cl, epiweek2 == reporting_week, pt_outcome %in% c("death", "died")))`.
...
## Count cases by sector and block in the past 4 weeks
...
ggplot() +
  ...
  # label polygons
  geom_sf_text(data = map_ajs_3weeks_cases, aes(label = bloc.x), colour = "black", size = 3) +
  geom_text(data = data.frame(), aes(label = 'Sector 1', x = 29.785, y = 9.339), angle = 10, vjust = 1) +
  geom_text(data = data.frame(), aes(label = 'Sector 2', x = 29.7895, y = 9.34), angle = 10, hjust =  0.8) +
  geom_text(data = data.frame(), aes(label = 'Sector 3', x = 29.80, y = 9.339, angle = -70, hjust = 1, vjust =  0.7)) +
  geom_text(data = data.frame(), aes(label = 'Sector 4', x = 29.8011, y = 9.3348, angle = -70, hjust = 1, vjust = - 0.1)) +
  geom_text(data = data.frame(), aes(label = 'Sector 5', x = 29.8025, y = 9.33, angle = -70, hjust = 1, vjust = - 0.45)) +
...
breakers <- as.integer(c(
                         ...
                         find_breaks(n = max_ajs_2020_cases, breaks = 4, snap = 2)
             ))
...
ggplot() +
  ...
  # scale_x_discrete( breaks = breakers$brks) +
  # label polygons
  geom_sf_text(data = map_ajs_2020_cases, aes(label = bloc.x), colour = "black", size = 3) +
  geom_text(data = data.frame(), aes(label = 'Sector 1', x = 29.785, y = 9.339), angle = 10, vjust = 1) +
  geom_text(data = data.frame(), aes(label = 'Sector 2', x = 29.7895, y = 9.34), angle = 10, hjust =  0.8) +
  geom_text(data = data.frame(), aes(label = 'Sector 3', x = 29.80, y = 9.339, angle = -70, hjust = 1, vjust =  0.7)) +
  geom_text(data = data.frame(), aes(label = 'Sector 4', x = 29.8011, y = 9.3348, angle = -70, hjust = 1, vjust = - 0.1)) +
...
## Plot weeks vs deaths by year
ggplot(measles_poc_cl_grp, aes(x = epiweek,
                               y = n, 
                               fill = as.factor(year))) +
  geom_col() +
  ## add the four week average as a line
...
ggplot() +
  ...
  geom_text(data = data.frame(), 
            aes(label = 'Sector 1', x = 29.785, y = 9.339), angle = 10, vjust = 1) +
  geom_text(data = data.frame(), 
            aes(label = 'Sector 2', x = 29.7895, y = 9.34), angle = 10, hjust =  0.8) +
  geom_text(data = data.frame(), 
            aes(label = 'Sector 3', x = 29.80, y = 9.339, angle = -70, hjust = 1, vjust =  0.7)) +
  geom_text(data = data.frame(),
            aes(label = 'Sector 4', x = 29.8011, y = 9.3348, angle = -70, hjust = 1, vjust = - 0.1)) +
  geom_text(data = data.frame(), 
            aes(label = 'Sector 5', x = 29.8025, y = 9.33, angle = -70, hjust = 1, vjust = - 0.45)) +
  # remove coordinates and axes
  theme_void() +
  labs(fill = "No. of cases",
       captions = str_glue("Source: MSF data from {start_year} to {reporting_week}"),
