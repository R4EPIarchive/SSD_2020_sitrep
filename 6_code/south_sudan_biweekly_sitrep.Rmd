...
ajs_poc_clean <- ajs_poc_raw %>%
  ...
  ## convert sn to integer
  mutate(sn = as.integer(sn)) %>% 
  ## Add year variables
  mutate(year = ifelse(sn <= 127, 2019, 2020)) %>% 
  ## convert epi_week to integer
  mutate(epi_week = as.integer(epi_week))
...
ggplot(bentiu_last_current, aes(x = week, y = total_deaths, 
                                fill = as.factor(year))) +
  geom_col( ) +
  ## add the four week average as a line
  geom_line(aes(x = week, y = four_week_roll), 
            size = 1.2) +
  ## facet by year
  facet_grid(year ~ .) +
...
There were `r fmt_count(ajs_poc_clean, is.na(sex)| sex == "Unknown/unspecified")` cases missing information on sex and `r fmt_count(measles_poc_cl, is.na(age_group))` missing age group.
...
ggplot(ajs_poc_clean_grp, aes(x = epi_week, y = n, 
                                fill = as.factor(year))) +
  geom_col( ) +
  ## add the four week average as a line
  geom_line(aes(x = epi_week, y = four_week_roll), 
            size = 1.2) +
  ## facet by year
  facet_grid(year ~ .) +
